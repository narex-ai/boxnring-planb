...
from app.api.v1.dependencies import get_visitor_onboarding, _format_respond
...
async def visitor():
    """
    GET endpoint for visitor onboarding that calls Gemini with a specific prompt
    ...
    try:
        ...
        template = get_visitor_onboarding()
        ...
        logger.info("Visitor Onboarding Gemini call completed successfully")
        
        return {
            "status": "success",
            "message": "Visitor Onboarding Questions loaded successfully",
            "body": respond_body
        }
        
    except Exception as e:
        logger.error(f"Error in visitor onboarding: {e}", exc_info=True)
